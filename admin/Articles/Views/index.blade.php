...
<div class="px-4 sm:px-6 lg:px-8">
  @if($articles->isEmpty())
    <div class="text-center">
      ...
      <h3 class="mt-2 text-sm font-semibold text-gray-900 dark:text-white">No articles</h3>
      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Get started by creating a new article.</p>
      <div class="mt-6">
        <a href="{{ route('admin.articles.create') }}" class="inline-flex items-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 dark:bg-indigo-500 dark:shadow-none dark:hover:bg-indigo-400 dark:focus-visible:outline-indigo-500">
  ...
  <div class="sm:flex sm:items-center">
    <div class="sm:flex-auto">
      <h1 class="text-base font-semibold text-gray-900 dark:text-white">Articles</h1>
      <p class="mt-2 text-sm text-gray-700 dark:text-gray-300">A list of all the articles.</p>
    </div>
    <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
      <a href="{{ route('admin.articles.create') }}" class="block rounded-md bg-indigo-600 px-3 py-2 text-center text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 dark:bg-indigo-500 dark:hover:bg-indigo-400 dark:focus-visible:outline-indigo-500">Add article</a>
    </div>
  </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Sortable(sortableTableBody, {
        ...
        ghostClass: '.article', // Class for the ghost element
        onEnd: function (evt) {
    ...
    function saveOrderToServer(orderData) {
        ...
        fetch('{{ route('admin.articles.reorder') }}', { // Replace with your actual route
            method: 'POST',
            headers: {
