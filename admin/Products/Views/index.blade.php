...
<div class="px-4 sm:px-6 lg:px-8">
  @if($products->isEmpty())
    <div class="text-center">
      ...
      <div class="mt-6">
        <a href="{{ route('admin.products.create') }}" class="inline-flex items-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 dark:bg-indigo-500 dark:shadow-none dark:hover:bg-indigo-400 dark:focus-visible:outline-indigo-500">
  ...
  <div class="sm:flex sm:items-center">
    ...
    <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
      <a href="{{ route('admin.products.create') }}" class="block rounded-md bg-indigo-600 px-3 py-2 text-center text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 dark:bg-indigo-500 dark:hover:bg-indigo-400 dark:focus-visible:outline-indigo-500">Add product</a>
    </div>
  </div>
              ...
              <tbody id="sortable" class="divide-y divide-gray-200 bg-white dark:divide-white/10 dark:bg-gray-800/50">
                @foreach ($products as $product )
                    <tr class="product" data-id="{{ $product->id }}">
                      ...
                      <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">{{ $product->title }}</td>
                      <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">{{ $product->slug }}</td>
                      ...
                      <td class="py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-6">
                        <a href="{{ route('admin.products.edit', $product) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Edit<span class="sr-only">, </span></a>
                      </td>
                    </tr>                 
                @endforeach               
              </tbody>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function saveOrderToServer(orderData) {
        ...
        fetch('{{ route('admin.products.reorder') }}', { // Replace with your actual route
            method: 'POST',
            headers: {
