  ...
  <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="space-y-12">
      ...
      @foreach ($locales as $locale )
        <div x-show="activeTab === '{{ $locale }}'" class="grid grid-cols-1 gap-x-8 gap-y-10 border-b border-gray-900/10 pb-12 md:grid-cols-3 dark:border-white/10">
          <div>
            ...
            <p class="mt-1 text-sm/6 text-gray-600 dark:text-gray-400">Product information that is translatable can be used to help you quickly identify and manage your categories.</p>
          </div>
          ...
          <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 md:col-span-2">
            ...
            <div class="col-span-full">
              ...
              @if ($errors->has('meta_keywords.*'))           
                <p id="meta-kewords-error" class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $errors->first('meta_keywords.*') }}</p>
              @endif
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 gap-x-8 gap-y-10 border-b border-gray-900/10 pb-12 md:grid-cols-3 dark:border-white/10">
          <div>
            <h2 class="text-base/7 font-semibold text-gray-900 dark:text-white">Common Category Information</h2>
            <p class="mt-1 text-sm/6 text-gray-600 dark:text-gray-400">Common Category information that is not translatable can be used to help you quickly identify and manage your categories.</p>
          </div>
          ...
          <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 md:col-span-2">
            ...
            <div class="sm:col-span-3">
              ...
              @if ($errors->has('slug'))           
                <p id="slug-error" class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $errors->first('slug') }}</p>
              @endif
            </div>
            ...
            <div class="sm:col-span-3">
              <div class="flex items-center gap-x-3">
                <div class="group relative inline-flex w-11 shrink-0 rounded-full bg-gray-200 p-0.5 inset-ring inset-ring-gray-900/5 outline-offset-2 outline-indigo-600 transition-colors duration-200 ease-in-out has-checked:bg-indigo-600 has-focus-visible:outline-2 dark:bg-white/5 dark:inset-ring-white/10 dark:outline-indigo-500 dark:has-checked:bg-indigo-500">
                  ...
                  <input id="new-product" type="checkbox" name="new_product" aria-labelledby="new-product-label" aria-describedby="new-product-description" class="absolute inset-0 appearance-none focus:outline-hidden" />
                </div>
                ...
                @if ($errors->has('new_product'))           
                  <p id="new_product-error" class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $errors->first('new_product') }}</p>
                @endif
            </div>
            ...
            <div class="sm:col-span-3">
              <div class="flex items-center gap-x-3">
                <div class="group relative inline-flex w-11 shrink-0 rounded-full bg-gray-200 p-0.5 inset-ring inset-ring-gray-900/5 outline-offset-2 outline-indigo-600 transition-colors duration-200 ease-in-out has-checked:bg-indigo-600 has-focus-visible:outline-2 dark:bg-white/5 dark:inset-ring-white/10 dark:outline-indigo-500 dark:has-checked:bg-indigo-500">
                  <span class="size-5 rounded-full bg-white shadow-xs ring-1 ring-gray-900/5 transition-transform duration-200 ease-in-out group-has-checked:translate-x-5"></span>
                  <input id="published" type="checkbox" name="published" aria-labelledby="published-label" aria-describedby="published-description" class="absolute inset-0 appearance-none focus:outline-hidden" />
                </div>
                ...
                @if ($errors->has('published'))           
                  <p id="published-error" class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $errors->first('published') }}</p>
                @endif
            </div>
            ...
            <div class="col-span-full">
              ...
              <div class="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10 dark:border-white/25">
                  <div class="text-center">
                      ...
                      <svg id="upload-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" class="mx-auto size-12 text-gray-300 dark:text-gray-600">
                          <path d="M1.5 6a2.25 2.25 0 0 1 2.25-2.25h16.5A2.25 2.25 0 0 1 22.5 6v12a2.25 2.25 0 0 1-2.25 2.25H3.75A2.25 2.25 0 0 1 1.5 18V6ZM3 16.06V18c0 .414.336.75.75.75h16.5A.75.75 0 0 0 21 18v-1.94l-2.69-2.689a1.5 1.5 0 0 0-2.12 0l-.88.879.97.97a.75.75 0 1 1-1.06 1.06l-5.16-5.159a1.5 1.5 0 0 0-2.12 0L3 16.061Zm10.125-7.81a1.125 1.125 0 1 1 2.25 0 1.125 1.125 0 0 1-2.25 0Z" fill-rule="evenodd" clip-rule="evenodd" />
                      </svg>
