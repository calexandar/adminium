...
<div class="px-4 sm:px-6 lg:px-8">
  @if($pages->isEmpty())
    <div class="text-center">
      ...
      <h3 class="mt-2 text-sm font-semibold text-gray-900 dark:text-white">No pages</h3>
      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Get started by creating a new item.</p>
      <div class="mt-6">
        <a href="{{ route('admin.pages.create') }}" class="inline-flex items-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 dark:bg-indigo-500 dark:shadow-none dark:hover:bg-indigo-400 dark:focus-visible:outline-indigo-500">
  ...
  <div class="sm:flex sm:items-center">
    <div class="sm:flex-auto">
      <h1 class="text-base font-semibold text-gray-900 dark:text-white">Page</h1>
      <p class="mt-2 text-sm text-gray-700 dark:text-gray-300">A list of all the pages.</p>
    </div>
    <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
      <a href="{{ route('admin.pages.create') }}" class="block rounded-md bg-indigo-600 px-3 py-2 text-center text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 dark:bg-indigo-500 dark:hover:bg-indigo-400 dark:focus-visible:outline-indigo-500">Add Page</a>
    </div>
  </div>
              ...
              <tbody id="sortable" class="divide-y divide-gray-200 bg-white dark:divide-white/10 dark:bg-gray-800/50">
                @foreach ($pages as $item )
                    <tr class="item" data-id="{{ $item->id }}">
                      ...
                      <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">{{ $item->title }}</td>
                      <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">{{ $item->slug }}</td>
                      ...
                      <td class="py-4 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-6">
                        <a href="{{ route('admin.pages.edit', $item) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Edit<span class="sr-only">, </span></a>
                        <span>/</span>
                        <div x-data="{}" class="inline-block">
                          <form x-ref="deleteForm" :action="`{{ route('admin.pages.destroy', $item) }}`" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="button"
                              @click="Swal.fire({
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Sortable(sortableTableBody, {
        ...
        ghostClass: '.item', // Class for the ghost element
        onEnd: function (evt) {
    ...
    function saveOrderToServer(orderData) {
        ...
        fetch('{{ route('admin.pages.reorder') }}', { // Replace with your actual route
            method: 'POST',
            headers: {
